...
static uint8_t *md5_message_extend(size_t *size);
...
ft_ssl_status_t md5(uint8_t *message, size_t size, uint8_t digest[MD5_DIGEST_SIZE]) {
    ...
    md5_message = md5_message_pad(message, &size);
    if (md5_message == NULL) {
        return FT_SSL_ERROR;
    }
    md5_initialize(&state);
    md5_process_message(&state, md5_message, size);
    free(md5_message);
...
static uint8_t *md5_message_pad(uint8_t *message, size_t *size) {
    ...
    old_size = *size;
    padded_message = md5_message_extend(size);
    if (padded_message == NULL) {
        return NULL;
    }
    memcpy(padded_message, message, old_size);
    md5_message_fill(padded_message + old_size, *size - old_size);
    md5_message_append_size(old_size, padded_message + *size - 8);
