...
static uint8_t *sha256_message_extend(uint8_t *message, size_t *size);
...
static void sha256_process_block(const uint8_t *block, sha256_state_t *state) {
    uint32_t W[SCHEDULE_SIZE_U32];
    uint32_t t1, t2;

    sha256_schedule(W, block);
    for (size_t i = 0; i < 64; i++) {
        t1 = state->h + BSIG1(state->e) + CH(state->e, state->f, state->g) + g_sha256_k[i] + W[i];
...
static uint8_t *sha256_message_pad(uint8_t *message, size_t *size) {
    ...
    old_size = *size;
    message = sha256_message_extend(message, size);
    padding = *size - old_size;
    if (message == NULL) {
        return NULL;
    }
    sha256_message_fill(message + old_size, padding);
    sha256_message_append_size(old_size, message + *size - 8);
    return message;
}
...
static uint8_t *sha256_message_extend(uint8_t *message, size_t *size) {
    ...
    //TODO: Don´t use realloc since sha256 don´t hold the responsability of freeing the message
